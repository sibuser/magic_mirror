...
class Currency(BaseModule):
    ...
    def show_rate(self):
        ...
        position = surface.get_rect(left=self.width / 24, top=self.height * 0.5)
        ...
        position = surface.get_rect(left=self.width / 7, top=self.height * 0.5)
        self.new_data.append((surface, position))

        surface = self.font('regular', 0.025).render('USD/RUB', True, self.color)
        position = surface.get_rect(left=self.width / 24, top=self.height * 0.55)
        self.new_data.append((surface, position))

        rate = CurrencyConverter().convert(1, 'USD', 'RUB')
        surface = self.font('regular', 0.025).render(str(round(rate, 2)), True, self.color)
        position = surface.get_rect(left=self.width / 7, top=self.height * 0.55)
        self.new_data.append((surface, position))
